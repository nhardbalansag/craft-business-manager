...
function setup(
  ...
  const requirementProvider = { plan: vi.fn().mockResolvedValue(requirementResult) };
  const costProvider = { previewForProduct: vi.fn().mockResolvedValue(costResult) };
  const service = new WasteAdjustedDirectMaterialCostService(requirementProvider, costProvider);
  return { service, requirementProvider };
}
...
describe('WasteAdjustedDirectMaterialCostService', () => {
  ...
  it('keeps a zero safety-waste reserve as explicit zero cost', async () => {
    const req = requirement('wax', { effective: 100, reserve: 0, planned: 100, safetyWasteRate: 0 });
    const { service } = setup(
      plan([req], { safetyWasteRate: 0, safetyWastePercentage: 0, safetyWasteMultiplier: 1 }),
  ...
  it('aggregates multiple materials and sorts lines deterministically', async () => {
    ...
    const { service } = setup(
      plan([beta, alpha]),
      preview([costLine('beta', 4, 2), costLine('alpha', 5, 3)]),
    );
  ...
  it('uses precise plannedBaseQuantityPerProduct and ignores one-piece pc batch rounding', async () => {
    const wick = requirement('wick', { baseUnit: 'pc', effective: 0.4, reserve: 0.04, planned: 0.44, batch: 1 });
    const { service } = setup(plan([wick]), preview([costLine('wick', 10, 0.4, 'pc')]));
  ...
  it('does not multiply the safety reserve a second time', async () => {
  ...
  it('preserves known cost evidence while requirement readiness is partial', async () => {
    const req = requirement('wax');
    const requirementResult = plan([req], {
      status: 'partial',
      issues: [{ code: 'YIELD_HISTORY_NOT_DERIVABLE', message: 'Some yield evidence is invalid.' }],
    });
    ...
    expect(result.pricingDirectMaterialCostPerUnit).toBeCloseTo(2.2);
    expect(result.issues).toContainEqual(expect.objectContaining({ code: 'REQUIREMENT_PARTIAL' }));
    expect(result.requirementIssues).toEqual(requirementResult.issues);
  });

  it('keeps a known subtotal when one planned material lacks cost evidence', async () => {
    const wax = requirement('wax');
    const dye = requirement('dye', { effective: 5, reserve: 0.5, planned: 5.5 });
    const { service } = setup(plan([wax, dye]), preview([costLine('wax', 0.02)], { status: 'partial' }));
    const result = await service.costProduct('Product-A');
    ...
    expect(result.issues).toContainEqual(expect.objectContaining({ code: 'MATERIAL_COST_EVIDENCE_MISSING', materialId: 'dye' }));
  });

  it('preserves the existing no-requirements state as not-ready', async () => {
    const requirementResult = plan([], {
      status: 'not-ready',
      issues: [{ code: 'NO_REQUIREMENTS', message: 'No direct requirements are derivable.' }],
    });
    const { service } = setup(
      requirementResult,
      preview([], { status: 'not-ready', requirementStatus: 'not-ready', requirementIssues: requirementResult.issues }),
    );
    const result = await service.costProduct('Product-A');
  ...
  it('joins material evidence case-insensitively and preserves requirement identity', async () => {
    const req = requirement('Wax-A');
    const { service } = setup(plan([req]), preview([costLine('  wax-a  ', 0.02)]));
    const result = await service.costProduct('Product-A');
    expect(result.status).toBe('ready');
    expect(result.lines[0]?.materialId).toBe('Wax-A');
  });

  it('fails a base-unit mismatch closed', async () => {
    const req = requirement('wax', { baseUnit: 'g' });
    const { service } = setup(plan([req]), preview([costLine('wax', 0.02, 100, 'mL')]));
    const result = await service.costProduct('Product-A');

    expect(result.status).toBe('not-ready');
    expect(result.pricingDirectMaterialCostPerUnit).toBeNull();
    expect(result.issues).toContainEqual(expect.objectContaining({ code: 'MATERIAL_BASE_UNIT_MISMATCH', materialId: 'wax' }));
  });

  it('surfaces unmatched cost evidence as partial integrity evidence', async () => {
    const req = requirement('wax');
    const { service } = setup(plan([req]), preview([costLine('wax', 0.02), costLine('ghost', 1, 1)]));
    const result = await service.costProduct('Product-A');

    expect(result.status).toBe('partial');
    expect(result.issues).toContainEqual(expect.objectContaining({ code: 'MATERIAL_REQUIREMENT_EVIDENCE_MISSING', materialId: 'ghost' }));
  });
  ...
  it('throws a typed error when requirement and cost evidence belong to different Products', async () => {
    ...
    await expect(service.costProduct('Product-A')).rejects.toBeInstanceOf(WasteAdjustedDirectMaterialCostServiceError);
  ...
  it('fails invalid or unreconciled derived cost evidence closed', async () => {
    const invalid = requirement('invalid');
    const badReconciliation = requirement('reconcile', { effective: 1, reserve: 0.1, planned: 1.2 });
    const { service } = setup(
      plan([invalid, badReconciliation]),
      preview([costLine('invalid', Number.POSITIVE_INFINITY), costLine('reconcile', 10, 1)]),
    );
    const result = await service.costProduct('Product-A');
  ...
  it('defensively clones source issues and nested contribution evidence', async () => {
    ...
    const requirementResult = plan([req], {
      status: 'partial',
      issues: [{ code: 'YIELD_HISTORY_NOT_DERIVABLE', message: 'Original requirement issue.' }],
    });
    const line = costLine('wax', 0.02);
    const costResult = preview([line], {
      status: 'partial',
      costIssues: [{ code: 'MATERIAL_COST_NOT_DERIVABLE', materialId: 'other', message: 'Original cost issue.' }],
    });
