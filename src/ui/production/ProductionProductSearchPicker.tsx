...
import './productionProductSearchPicker.css';

const PRODUCT_RESULT_LIMIT = 8;

export interface ProductionProductSearchPickerProps {
...
export function ProductionProductSearchPicker({
  products,
  selectedProductId,
  disabled = false,
  onSelect,
}: ProductionProductSearchPickerProps) {
  ...
  return (
    <div
      className="production-product-picker"
      onBlur={(event) => {
        if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setOpen(false);
      }}
    >
      <label className="field production-product-search-field">
        <span>Product to make</span>
        <div className="production-product-search-input-wrap">
          <span className="production-product-search-icon" aria-hidden="true">
            <AppIcon name="search" size={18} />
          </span>
          <input
            type="search"
            role="combobox"
            aria-label="Search product to make"
          ...
          {query && !disabled && (
            <button
              type="button"
              className="production-product-search-clear"
              aria-label="Clear production product search"
              onClick={() => {
                setQuery('');
                setOpen(true);
                setActiveIndex(0);
        ...
        <small>
          {selectedProduct
            ...
            : 'Search for the product you want to plan and make.'}
        </small>
      </label>

      {open && !disabled && (
        <div className="production-product-search-menu">
          <div className="production-product-search-menu-heading">
            <span>{query.trim() ? 'Matching products' : 'Products'}</span>
            <strong>{matchingProducts.length}</strong>
          </div>
          <div
            id={listboxId}
            className="production-product-search-results"
            role="listbox"
            aria-label="Production product search results"
          >
            {visibleProducts.length === 0 ? (
              <div className="production-product-search-empty" role="status">
              ...
              <button
                ...
                className={`production-product-search-result ${index === activeIndex ? 'is-keyboard-active' : ''} ${product.id === selectedProductId ? 'is-selected' : ''}`}
                onMouseEnter={() => setActiveIndex(index)}
                onClick={() => choose(product)}
              >
                <span className="production-product-search-result-main">
                  <strong>{product.name}</strong>
                  <small>{product.id} · {PRODUCT_CATEGORY_RULES[product.category].label}</small>
                </span>
                <span className={`production-product-search-status ${product.isActive ? 'is-active' : 'is-archived'}`}>
          ...
          {matchingProducts.length > PRODUCT_RESULT_LIMIT && (
            <div className="production-product-search-more">
