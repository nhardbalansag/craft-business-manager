...
Baseline:
- `develop`: `144b75751543af530aa1d93593e84598087e5065`
- CI: `35156618396 — SUCCESS`

## Goal

Add a first Google Sheets integration that imports a publicly published Google spreadsheet into Craft Business Manager without Google sign-in, OAuth, API keys, or a second persistence model.
...
- Fetch with browser credentials omitted so the feature cannot silently depend on a signed-in Google session.
- Keep fetched bytes pending until explicit user confirmation.
...
- Make clear that this version is read-only snapshot import, not live sync and not write-back.
...
- App integration coverage proving successful Google Sheets import updates the compact Workbook identity and workspace revision.
...
- Arbitrary CSV/table layouts that do not match the Craft Business Manager workbook contract.
...
A successful public Google Sheets fetch does not change live app data. Only explicit Apply Import may hydrate repositories. Rejected or failed imports preserve current live data. The browser capability makes no claim that a public Google Sheet is private, authenticated, writable, synchronized, or backed up.
